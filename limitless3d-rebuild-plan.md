...
2. **Testimonials / reviews** — quote, attribution, source (Google/Etsy). Optionally seeded
   ...
   *Built 2026-08-12 (session 8, D-039/D-040): the hybrid is live — curated collection
   untouched, auto strip + live proof-bar values added; fixture mode until the API key
   clears approval.*
...
- Square catalog integration (§2) is now pre-cutover scope (2026-08-09, §10): product pages rendered from the Catalog API with Square-hosted checkout links, using the prototype's shop design as reference (snapshot: `shop.html` + `parts-pdp-example.html`).
  *Done 2026-08-12 (session 7, D-034–D-036): inventory extracted verbatim (61 products,
  ...
  61 PDPs building from the catalog with per-variation payment links, fake checkout
  COMPLETED in sandbox, staging re-verified. Nothing links to /parts yet — the
  SHOP_URL flip is an open proposal. Cutover = re-extract → seed production → env swap
  (§9.7 Path A).*
...
8. Etsy reviews auto-pull (§10, 2026-08-09): does Randy want it, and will he register an Etsy
   ...
   *Re-scoped 2026-08-12 (Ryan; same move as §9.7): the build no longer waits on Randy —
   the reviews module gets built on Ryan's own Etsy developer app (public-data reads),
   sequenced as session 8 ahead of Turnstile/CI. Randy's go becomes a walkthrough
   validation; whether the key later moves to a shop-account app is a cutover-time swap.
   The session verifies current Etsy API docs before building — the 2026-08-09
   investigation is eleven months old.*
   *Correction from that verification (2026-08-12, session 8 §10 row): "provisional API
   keys work immediately" is no longer true — new keys sit **Pending** until manually
   approved (typically 1–2 weekdays, can stall for weeks; writing to developer@etsy.com
   unsticks it). Register first, build against a fixture while waiting.*
   *Executed 2026-08-12 (session 8, D-039/D-040): app "limitless-3d" registered under
   Ryan's account (pending approval), module + hybrid wiring live on staging in fixture
   mode (10 verbatim reviews recorded from the public shop page; shop id 26921666).
   Remaining: the live flip when the key clears (env change + shop-id re-resolution +
   cross-check, RESUME has the steps). Randy's go stays a walkthrough validation; the
   open question is now only whether the key later moves to a shop-account app at
   cutover.*
...
| 2026-08-11 | **Square catalog un-blocked via Ryan's sandbox (Ryan).** The §9.7 credentials gate falls: the catalog integration gets built and demoed against Ryan's own Square account in **sandbox** mode, seeded with Randy's public product data (names/prices/photos from the live shop + the D-024 snapshot) via a one-off Catalog API script. Sandbox supports complete fake checkouts (test card), so Randy's walkthrough shows his actual products in the new design with a full purchase flow — while proving the core promise: the site only *renders* the catalog; delivery/costs/payments stay in whatever Square account is connected. Randy's real credentials become a cutover-time env swap (token + location id), not a build dependency. Demo catalog is a snapshot, not live-synced — retired when real credentials land. Session order while Randy is away (Ryan): Square catalog → Turnstile/CI/studio-hosting → 1a polish + improvements. |
| 2026-08-12 | **Etsy reviews promoted to session 8; roadmap re-sequenced (Ryan).** The §9.8 gate falls the way §9.7's did: build against Ryan's own Etsy developer app (public shop data, API-key reads), Randy validates at the walkthrough. Scope per the 2026-08-09 investigation's hybrid: proof-bar aggregate auto-feeds from the shop resource (which stats flip from the Sanity singleton vs stay owner-edited is a session judgment call), curated §4.2 testimonials keep the quote band and existing cards, a 5★-with-text pool adds a recent-reviews strip on /reviews inside the existing design vocabulary, truthful aggregate always beside filtered quotes (FTC framing), Etsy attribution per their terms. Build-time fetch means reviews refresh only on deploy → the CI session (now 9) gains a weekly scheduled rebuild alongside deploy-on-push and Sanity webhook. New order: 8 = Etsy reviews, 9 = Turnstile + CI + studio hosting, 10 = 1a polish. Session 8 must re-verify current Etsy API reality (registration flow, key-only auth for `getReviewsByShop`, display/attribution policy) before building — the investigation is from 2026-08-09. |
| 2026-08-12 | **Etsy API reality re-verified (session 8 step 0; sources: current OpenAPI spec `etsy.com/openapi/generated/oas/3.0.0.json`, developer docs, API Terms of Use snapshot 2026-08-01).** Four deltas against the 2026-08-09 investigation, none fatal: (1) **Keys no longer work immediately** — new apps sit "Pending" until manually approved (docs: "not active until it has been approved"; community/GitHub Jan 2026: 1–2 weekdays typical, can stall for weeks, emailing developer@etsy.com unsticks it). Sequencing flip: Ryan registers first thing, the module gets built and verified against a recorded fixture while the key waits, live verification happens when it clears. (2) **`Shop.review_count`/`review_average` are past-YEAR numbers** (per the spec's own field descriptions), not the lifetime "4.8 / 500+" the public shop page shows — so the truthful aggregate is instead computed from `getReviewsByShop` itself by paging the full review set (100/page, ~6 requests; response `count` is the total). `transaction_sold_count` IS lifetime. (3) **Current API Terms impose a display-freshness ceiling: non-listing Etsy content may not be displayed more than 24 hours staler than Etsy shows it** (listing content: 6h) — the session-9 scheduled rebuild becomes **daily, not weekly**. (4) Exact attribution text required, verbatim, prominently displayed: "The term 'Etsy' is a trademark of Etsy, Inc. This Application uses Etsy's API, but is not endorsed or certified by Etsy." — plus Etsy branding must stay less prominent than our own. Confirmed unchanged: `getReviewsByShop`/`getShop`/`findShops` are API-key-only (global `api_key` security, no OAuth scope), reviews carry rating/text/timestamps/photo but **no reviewer display name** (buyer_user_id nullable), limit max 100 with offset paging + min/max_created filters, shop id resolvable via `findShops?shop_name=`. One ambiguity to settle empirically at first live call: the spec's securityScheme says `x-api-key: keystring:shared_secret` while established practice is keystring alone. Rate limits are now per-app in the developer portal (not published tiers) — irrelevant at ~8 build-time requests. |
| 2026-08-12 | **Etsy reviews live in fixture mode (Phase 1b session 8; D-039/D-040).** The §10 2026-08-12 promotion row executed with one twist from step 0's verification: keys no longer activate on registration, so Ryan's app sits "Pending Personal Approval" and the session shipped fixture-first — `src/data/etsy.ts` (cms/square contract; live path fully written, pages the whole review set key-only) reading 10 reviews recorded verbatim from the public shop page, so staging shows only real publicly-displayed content. Hybrid per the investigation: proof-bar rows flip live by singleton key (rating = Etsy's own recency-weighted average, counts floored into the "N+" vocabulary — never overstates; Star Seller stays owner-edited, no API source), /reviews gains the "Straight from Etsy" strip (4 newest 5★, whole-or-not-featured, truthful aggregate in the sub line, required trademark notice verbatim), curated Sanity testimonials untouched with an explicit revert path (Ryan). Staging re-verified from the public URL. Owed: the live flip on key approval (RESUME) and session 9's daily cron + `ETSY_*` CI secrets. |
| 2026-08-12 | **Square catalog demo live (Phase 1b session 7; D-034–D-036).** The §10 2026-08-11 sandbox plan executed end to end: 61 products extracted verbatim (the live shop's own public storefront API is canonical for copy/prices/galleries; the D-024 snapshot supplies the editorial machine/bucket/order layer; slug = pure function of name), Ryan's sandbox seeded via an idempotent Catalog API script (also the §9.7 Path A production migration tool — Randy's `square_id: null` products mean his production catalog is likely empty), `/parts` + 61 PDPs fetched at build with per-variation Square-hosted payment links (ensure-semantics: the build's one write; new products self-link on the next build), fake checkout COMPLETED in sandbox and confirmed via the Payments API, staging redeployed and re-verified (noindex intact, zero hotlinks). Platform lessons pinned in code: Square-Version header (unpinned = descriptions silently dropped), content-dedup on image upload, item upserts replace children and clear omitted `image_ids`. Interim state preserved: all "Parts Shop" links still point at `SHOP_URL` (D-011) — the /parts flip is proposed for Ryan's decision, not assumed. |
